...
<body style="zoom:75%;">
	<div class="header1">
		<h3 style="text-align: center;" >Table Activities</h3>
	</div>

    <p class="text">Συμπληρώστε τα παρακάτω πεδία και πατήστε <span>SUBMIT</span> για να εισάγετε στοιχεία στην βάση.</p>
	
<div class="formStyle">
		<form class="form-inline" onsubmit="pageReload();" action="doactivity_live_edit.php" method="post" target="dummyframe">
				...
				<input type="num" name="Duration" id="Duration" placeholder="Duration" required>
	...
	<input type="button" value="Refresh Table" onClick="pageReload();">
		...
		<table style="width:100%;" id="data_table" class="table table-striped">
			...
			$sql_query = "SELECT ID_Pet,ID_Room,ATEmployee,Date,Duration,Type FROM doactivity";
			$resultset = mysqli_query($conn, $sql_query) or die("database error:". mysqli_error($conn));
			while( $data = mysqli_fetch_assoc($resultset) ) {
			?>
			<?php
			 $ALL = "". $data ['ID_Pet']."/". $data ['ID_Room']."/". $data ['ATEmployee']."";
			?>
...
columns: {
	...
	identifier: [0, 'ALL'],
	editable: [[4,'Date'], [5,'Duration'], [ 6,'Type']]
	},
